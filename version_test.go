...
func TestVersion_VersionFromString(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expected: &Version{major: 1, minor: 2, patch: 3, pre: "alpha.1", build: "build.2"},
		},
		{
			title:    "parses simple major",
			input:    "42",
			expected: &Version{major: 42},
		},
		{
			title:    "parses major.minor",
			input:    "42.24",
			expected: &Version{major: 42, minor: 24},
		},
		{
			title:    "parses a major.minor.patch",
			input:    "42.24.9",
			expected: &Version{major: 42, minor: 24, patch: 9},
		},
		...
		// Pre-release examples from spec:
		{
			...
			input:    "1.0.0-alpha",
			expected: &Version{major: 1, pre: "alpha"},
		},
		{
			title:    "pre-release 1.0.0-alpha.1",
			input:    "1.0.0-alpha.1",
			expected: &Version{major: 1, pre: "alpha.1"},
		},
		{
			title:    "pre-release 1.0.0-0.3.7",
			input:    "1.0.0-0.3.7",
			expected: &Version{major: 1, pre: "0.3.7"},
		},
		{
			title:    "pre-release 1.0.0-x.7.z.92",
			input:    "1.0.0-x.7.z.92",
			expected: &Version{major: 1, pre: "x.7.z.92"},
		},
		{
			title:    "pre-release 1.0.0-x-y-z.--",
			input:    "1.0.0-x-y-z.--",
			expected: &Version{major: 1, pre: "x-y-z.--"},
		},
